		...
		<view class="step_one" v-if="step == 1">
			...
			<view class="popup_group">
				...
				<view class="popup_group_item" @click="selectBrand">
					...
					<view class="popup_group_item_value">
						<view class="popup_group_item_message">
							<span v-if="setFormData.brand_name" class="popup_group_item_message_value">{{ setFormData.brand_name }}</span>
							<span v-else>请选择商品品牌</span>
						</view>
						<view><span class="iconfont">&#xe6bd;</span></view>
					</view>
				</view>
	...
	data() {
		return {
			...
			setFormData: {
				...
				setSpecificaValue2: '',
				delivery_way: [], // 配送方式 1 到店自提 2 快递配送
			...
			res => {
			  ...
			  if (that.deliveryType.length == 2) {
			    ...
			    if (that.deliveryType.length == 1 && that.deliveryType[0] == '1') {
			        that.deliveryFreeList = [{ value: '1', name: '到店自提' }]
			    } else {
		...
		initDataEditData() {
			...
			productDetail(this.merId, this.product_id).then(res => {
				...
				let editGoodsDetils = res.data;
					...
					this.setFormData.mer_cate_name = editGoodsDetils.merCateId ? editGoodsDetils.merCateId.map(item => item.category.cate_name).join(',') : '';
