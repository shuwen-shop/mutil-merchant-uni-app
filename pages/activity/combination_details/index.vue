		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
						...
						<view class='wrapper'>
							<view class='share acea-row row-between row-bottom' style="padding: 0;margin: 0;">
								<view class='money skeleton-rect' style="min-width: 10rpx;">
									<text class="font-color">￥<text class='num'>{{combinationInfo.price}}</text></text>
			...
			<view class='footer_count'>
				...
				<view class="acea-row row-between-wrapper footer">
					...
					<navigator v-else hover-class="none" class="item  skeleton-rect" :url="`/pages/chat/customer_list/chat?mer_id=${storeInfo.mer_id}&uid=${uid}&productId=${storeInfo.product_id}&combinationId=${id}`">
						<view class="iconfont icon-kefu"></view>
						<view>客服</view>
					</navigator>
