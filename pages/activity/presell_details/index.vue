		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
			...
			<view class='footer_count'>
				...
				<view class="acea-row row-between-wrapper footer" :class="presellInfo.presell_type === 2 ? 'noborder' : ''">			
					...
					<navigator v-else hover-class="none" class="item skeleton-rect" :url="`/pages/chat/customer_list/chat?mer_id=${storeInfo.mer_id}&uid=${uid}&productId=${storeInfo.product_id}&presellId=${id}`">
						<view class="iconfont icon-kefu"></view>
						<view>客服</view>
					</navigator>
			...
			<block v-if="coupon.coupon">
				<couponListWindow :showTitle="0" :isShop="1" :coupon='coupon' @ChangCouponsClone="ChangCouponsClone" @ChangCoupons="ChangCoupons"
