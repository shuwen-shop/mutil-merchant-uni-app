	...
	<view>
		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
					...
					<view id="past0">
						...
						<view class='wrapper'>
							...
							<view class='label acea-row row-between-wrapper'>
								<view class="skeleton-rect">原价:￥{{storeInfo.ot_price}}</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<navigator v-else hover-class="none" class="item skeleton-rect" :url="`/pages/chat/customer_list/chat?mer_id=${storeInfo.mer_id}&uid=${uid}&productId=${id}`">
					<view class="iconfont icon-kefu"></view>
					<view>客服</view>
				</navigator>
	...
	export default {
		...
		methods: {
			...
			async goPoster() {
				...
				// #endif
				// #ifdef H5 
				arr2 = [that.posterbackgd, await that.imgToBase(that.storeImage), await that.imgToBase(this.codeImg)];
				// #endif
				console.log(arr2)
