		...
		<view class='payment-status'>
			...
			<view class='wrapper'>
				...
				<view v-if="order_pay_info.give_integral > 0" class='item acea-row row-between-wrapper'>
					<view>赠送积分</view>
					<view class='itemCom' style="color: #ee5a52;">{{order_pay_info.give_integral}}个</view>
				</view>
				<!--失败时加上这个  -->
			...
			<button @click="goPink(order_pay_info.activity_id)" class='returnBnt cart-color' formType="submit" hover-class='none' v-if="order_pay_info.activity_type == 4 && order_pay_info.paid">邀请好友参团</button>
			<button v-if="!order_pay_info.paid || order_pay_info.activity_type != 4" @click="goIndex" class='returnBnt cart-color' formType="submit" hover-class='none'>返回首页</button>
