				...
				<view class='item' v-for="(item,index) in orderList" :key="index">
					<view>
						<view class='title acea-row row-between-wrapper'>
							...
							<view v-if="item.presellOrder.activeStatus === 1" class='font-color'>等待买家付尾款</view>
							<view v-if="item.presellOrder.activeStatus === 0" class='font-color'>未开始</view>
							<view v-if="item.presellOrder.activeStatus === 2" class='font-color'>交易已关闭</view>
						</view>
						<block v-for="(goods,g) in item.orderProduct">
							...
							<view  class='item-info acea-row row-between row-top' @click='goOrderDetails(item.order_id)'>
								...
								<view v-if="goods.cart_info.productPresell.presell_type === 2" class="event_price">	
									...
									尾款待支付 <text class="font-color">￥{{ item.presellOrder.pay_price }}</text>
								</view>
							</view>																																
						</block>
						...
						<view class='bottom acea-row row-right row-middle'>
							...
							<view v-if="item.presellOrder.activeStatus === 1" class='bnt bg-color' @click.stop='goPay(item.presellOrder.pay_price,item.order_id)'>立即付款</view>
							<view v-if="item.presellOrder.activeStatus === 0" class='bnt bg-color btn_auto'>未开始</view>
							<view v-if="item.presellOrder.activeStatus === 2" class='bnt bg-color'>交易已关闭</view>
						</view>
					</view>	
				</view>	
			</view>
