<template>
	<view class="order-details pos-order-details">
		...
		<view class="address">
			...
			<!-- #endif -->
			<!-- #ifdef MP -->
			<!-- <button class="copy copy-data" @click="copyData('收货人姓名：'+orderInfo.real_name+'收货人电话：'+orderInfo.user_phone+'收货人地址：'+orderInfo.user_address)">复制</button> -->
		...
		<view class="pos-order-goods">
			<navigator :url="`/pages/goods_details/index?id=${item.cart_info.product.product_id}`" hover-class="none" class="goods acea-row row-between-wrapper" v-for="(item, index) in orderInfo.orderProduct" :key="index">
				...
				<view class="money">
					...
					<view class="y-money">￥{{ item.cart_info.productAttr.ot_price }}</view>
				</view>
			</navigator>
		</view>
		...
		<view class="wrapper" v-if="orderInfo.delivery_type != '3' && orderInfo.order_type === 0 && orderInfo.delivery_type">
			...
			<view class="item acea-row row-between">
				...
				<view class="conter">{{ orderInfo.delivery_name ? orderInfo.delivery_name : '' }}</view>
			</view>
			<view class="item acea-row row-between">
